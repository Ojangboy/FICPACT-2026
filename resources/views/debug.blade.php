...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        header { background: #16213e; padding: 16px 24px; display: flex; align-items: center; gap: 12px; border-bottom: 2px solid #0f3460; position: sticky; top: 0; z-index: 100; }
        header h1 { font-size: 1.4rem; color: #4ecca3; }
        #token-badge { margin-left: auto; font-size: 0.75rem; padding: 4px 12px; border-radius: 20px; background: #0f3460; color: #aaa; }
        #token-badge.active { background: #1a5c3a; color: #4ecca3; }
        .layout { display: flex; gap: 0; }
        nav { width: 200px; min-height: calc(100vh - 57px); background: #16213e; padding: 12px 0; position: sticky; top: 57px; overflow-y: auto; flex-shrink: 0; }
        nav a { display: block; padding: 10px 20px; color: #aaa; text-decoration: none; font-size: 0.85rem; border-left: 3px solid transparent; transition: all .2s; }
        nav a:hover, nav a.active { color: #4ecca3; border-color: #4ecca3; background: #0f3460; }
        nav .nav-section { padding: 12px 20px 4px; font-size: 0.7rem; color: #555; text-transform: uppercase; letter-spacing: 1px; }
        main { flex: 1; padding: 24px; overflow: auto; }
        .section { display: none; }
        .section.active { display: block; }
        .card { background: #16213e; border: 1px solid #0f3460; border-radius: 12px; padding: 20px; margin-bottom: 16px; }
        .card h3 { color: #4ecca3; margin-bottom: 14px; font-size: 1rem; display: flex; align-items: center; gap: 8px; }
        .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px; }
        label { display: block; font-size: 0.8rem; color: #aaa; margin-bottom: 4px; }
        input, select, textarea { width: 100%; padding: 8px 12px; background: #0f3460; border: 1px solid #1a4080; border-radius: 8px; color: #eee; font-size: 0.85rem; margin-bottom: 10px; outline: none; }
        input:focus, select:focus, textarea:focus { border-color: #4ecca3; }
        textarea { resize: vertical; min-height: 60px; }
        button { padding: 8px 20px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 0.85rem; transition: all .2s; }
        .btn-primary { background: #4ecca3; color: #1a1a2e; }
        .btn-primary:hover { background: #38b08a; }
        .btn-danger { background: #e74c3c; color: #fff; }
        .btn-danger:hover { background: #c0392b; }
        .btn-warn { background: #f39c12; color: #1a1a2e; }
        .btn-warn:hover { background: #d68910; }
        .response { margin-top: 12px; background: #0a0f1e; border: 1px solid #1a4080; border-radius: 8px; padding: 12px; }
        .response pre { font-size: 0.78rem; white-space: pre-wrap; word-break: break-all; color: #7ec8e3; }
        .response.ok pre { color: #4ecca3; }
        .response.err pre { color: #e74c3c; }
        .badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 0.7rem; font-weight: 700; }
        .badge-get { background: #1a5c3a; color: #4ecca3; }
        .badge-post { background: #1a3a5c; color: #4e9cca; }
        .badge-patch { background: #4a3a1a; color: #ca9c4e; }
        .badge-delete { background: #5c1a1a; color: #ca4e4e; }
        #pomodoro-timer { font-size: 3rem; font-weight: 900; color: #4ecca3; text-align: center; padding: 20px; letter-spacing: 4px; }
        #timer-bar { height: 8px; background: #0f3460; border-radius: 4px; margin: 8px 0 16px; }
        #timer-fill { height: 100%; background: #4ecca3; border-radius: 4px; width: 100%; transition: width 1s linear; }
        .task-item { background: #0f3460; border-radius: 8px; padding: 12px 16px; margin-bottom: 8px; display: flex; align-items: center; gap: 10px; }
        .task-item .task-title { flex: 1; }
        .task-item .task-badge { font-size: 0.7rem; padding: 2px 8px; border-radius: 10px; }
        .task-easy { background: #1a5c3a; color: #4ecca3; }
        .task-medium { background: #4a3a1a; color: #ca9c4e; }
        .task-hard { background: #5c1a1a; color: #ca4e4e; }
        .task-done { opacity: 0.5; text-decoration: line-through; }
        .streak-display { text-align: center; }
        .streak-display .streak-num { font-size: 3rem; font-weight: 900; color: #f39c12; }
        .streak-display .streak-label { color: #aaa; font-size: 0.85rem; }
    </style>
</head>
<body>
    ...
    <main>
            ...
            <div class="card">
                ...
                <div class="grid">
                    <div>
                        ...
                        <label>Confirm Password</label><input id="reg-confirm" type="password" placeholder="password123">
                        <button class="btn-primary" onclick="register()">Register</button>
                    </div>
                    <div class="response" id="reg-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="auth-login" class="section">
            <div class="card">
                ...
                <div class="grid">
                    <div>
                        <label>Email</label><input id="login-email" type="email" placeholder="john@example.com">
                        <label>Password</label><input id="login-password" type="password" placeholder="password123">
                        <button class="btn-primary" onclick="login()">Login</button>
                    </div>
                    <div class="response" id="login-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="auth-logout" class="section">
            <div class="card">
                ...
                <div class="response" id="logout-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="grid">
                    <div>
                        ...
                        <textarea id="refresh-token" placeholder="Paste refresh_token disini (otomatis terisi setelah login)"></textarea>
                        <button class="btn-primary" onclick="refreshToken()">Refresh</button>
                    </div>
                    <div class="response" id="refresh-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="user-me" class="section">
            <div class="card">
                ...
                <div class="response" id="user-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
        ...
        <div id="garden-show" class="section">
            <div class="card">
                ...
                <div class="response" id="garden-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
        ...
        <div id="task-list" class="section">
            <div class="card">
                ...
                <div class="response" id="task-list-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
        ...
        <div id="task-create" class="section">
            <div class="card">
                ...
                <div class="grid">
                    ...
                    <div class="response" id="task-create-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="task-complete" class="section">
            <div class="card">
                ...
                <div class="grid">
                    ...
                    <div class="response" id="task-complete-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="grid">
                    <div>
                        ...
                        <label>Description</label><textarea id="update-desc" placeholder="New description"></textarea>
                        <button class="btn-primary" onclick="updateTask()">Update Task</button>
                    </div>
                    <div class="response" id="task-update-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="task-delete" class="section">
            <div class="card">
                ...
                <div class="grid">
                    ...
                    <div class="response" id="task-delete-res"><pre>Response will appear here...</pre></div>
                </div>
            </div>
        </div>
        ...
        <div id="pomo-start" class="section">
            <div class="card">
                ...
                <p style="color:#aaa;font-size:0.85rem;margin-bottom:14px">Durasi: 25 menit. Timer akan berjalan otomatis setelah start.</p>
                <div id="pomodoro-timer">25:00</div>
                <div id="timer-bar"><div id="timer-fill"></div></div>
                <div style="text-align:center;margin-bottom:16px">
                    <button class="btn-primary" onclick="startPomodoro()" id="pomo-start-btn">▶ Start Pomodoro</button>
                </div>
                <div class="response" id="pomo-start-res"><pre>Response will appear here...</pre></div>
            </div>
        </div>
        ...
        <div id="pomo-finish" class="section">
            <div class="card">
                ...
                <p style="color:#aaa;font-size:0.85rem;margin-bottom:14px">Tandai sesi selesai. Streak akan bertambah.</p>
                <button class="btn-primary" onclick="finishPomodoro()">🏁 Finish Session</button>
                <div class="response" id="pomo-finish-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
        ...
        <div id="pomo-status" class="section">
            <div class="card">
                ...
                <div class="response" id="pomo-status-res" style="margin-top:12px"><pre>Response will appear here...</pre></div>
            </div>
        </div>
